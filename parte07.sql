...
BEGIN
    IF TG_OP = 'INSERT' THEN
		SELECT NEW.check_in + interval '1 month' AS inicio;--calculo inicio
		SELECT NEW.check_out + interval '2 years' AS fin;--calculo fin
		IF EXISTS (SELECT 1
		...
		WHERE ch.hotel_codigo = NEW.hotel_codigo AND
			  ...
			  ch.fecha_desde = (SELECT MAX(ch2.fecha_desde)
			  				    ...
			  				    WHERE (ch2.fecha_desde <= NEW.check_in) AND
			  				  	       ch2.hotel_codigo = NEW.hotel_codigo AND
		...
		SELECT NOW() AS timestamp_actual;--fecha actual
		IF timestamp_actual < fecha_fin THEN--si fecha actual menor a fin
